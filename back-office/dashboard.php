...
<body>
    ...
    <div id="wrap-content" class="content_show">
        <div class="area-content">
            ...
            <div>
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="sys_home/add_slide.php" method="post" enctype="multipart/form-data">
                            <div class="mb-3">
                                <label for="slide">เลือกรูปภาพ :</label>
                                <input class="form-control" type="file" name="slide" id="slide" required>
                            </div>
                            <div>
                                <label for="slide-mobile">เลือกรูปภาพ <span class="text-xs">*( สำหรับมือถือ )</span> :</label>
                                <input class="form-control" type="file" name="slide-mobile" id="slide-mobile" required>
                            </div>
                        ...
                        <?php
                        try {
                            $stmt = $db->conn->prepare("SELECT * FROM `slide_home` order by id desc");
                            $stmt->execute();
                            $stmt_count = $db->conn->prepare("SELECT COUNT(id) as _count FROM `slide_home` order by id desc");
                            $stmt_count->execute();
                                ...
                                <div class="mt-2">
                                    <div class="overflow-x-auto">
                                        <div class="flex items-end w-max">
                                            <div class="w-[100px]">
                                                <img class="w-full h-[50px] object-center object-cover border-[1px] border-zinc-500 cursor-pointer" src="images/slide_home/<?= $row["img"] ?>" alt="slide" billie-gallery="slide-<?= rand(1, 9999) ?>">
                                            </div>
                                            <div class="ml-3 w-[50px]">
                                                <img class="w-full h-[50px] object-center object-cover border-[1px] border-zinc-500 cursor-pointer" src="images/slide_home/<?= $row["img_mobile"] ?>" alt="slide-mobile" billie-gallery="slide-mobile-<?= rand(1, 1000) ?>">
                                            </div>
                                            <div class="ms-3">
    ...
    <script>
        function del_slide(id) {
            if (confirm(
                    "Are you want to delete image slide ?"
                )) {
                window.location.href = "sys_home/del_slide.php?id=" + id
            }
        }
    </script>
