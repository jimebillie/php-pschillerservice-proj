      ...
      <div class="box-main">
        ...
        <div class="show-title">
          ...
          ?>
              ...
              <?php
              try {
                $stmt = $db->conn->prepare("SELECT * FROM `portfolio_sub` WHERE `id`=:__id");
        ...
        <?php
        try {
          $stmt = $db->conn->prepare("SELECT * FROM `portfolio_sub_image` WHERE `id_sub`=:__id ORDER BY `id` DESC");
          $stmt->execute(["__id" => $_GET["id"]]);
          foreach ($stmt->fetchAll() as $row) {
        ?>
            <div class="box-port-sub-list">
              <div class="picture-port-sub-list">
                <img src="back-office/images/portfolio/<?=$row["img"]?>" width="400" height="267" />
              </div>
            </div>
        <?php
          }
        ...
        <div class="more-03"><a href="portfolio-list.php?id=<?=$_GET["id_main"]?>">
            < ย้อนกลับ</a>
        </div>
      </div>
